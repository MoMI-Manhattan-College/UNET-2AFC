...
You do not need to run this file, simply confirm that this file is imported to the main file (unets_run_files)
...
def get_mri_images(shuffle_seed=78, name = 'FLAIRVolumesSOS.mat', random_order = True):
    """Retrieves the MRI images

    This code retrieves the MRI image array that contains training and validation sets from 'FLAIRVolumesSOS.mat' currently
    (mri_images.mat before 7/24). Note that unlike mri_images.mat, these images are complex.
    ...
    This code returns the resulting images in an array. In our default file choice, there are 510 flair slices of shape 320 x 320.
    The number of images will most likely change but dimension 320 x 320 should not. See an earlier version
    of this file for importing the mri_images.mat, which have 1000 images of dimension 64 x 64.

    Args:
        shuffle_seed: This value fixes the random behavior of the function's data shuffling, so the same order results every time
        the same number is used. (78 chosen in summer 2019 by Marcus Wong)
        name: a string containing the name of the file we are uploading image from. The default is 'FLAIRVolumesSOS.mat',
...
def get_training_pic_sets(mri_images,undersampled_images,train_interval = (0,500)):
    ...
    although the resulting split could realy be used for any purpose.
