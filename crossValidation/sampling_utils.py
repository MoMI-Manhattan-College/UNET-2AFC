...
A collection of functions for creating, applying and displaying sampling fourier data sampling masks.
"""
...
from data_prep_utils import normalize_pictures
...
def get_sampling_mask(acceleration, dim):
    """Creates a general masking matrix

    This code assigns ones and zeros to the masking matrix, where ones will be sampled.
    Low k-space frequencies are sampled, including horizontal strips of higher frequencies, according to the acceleration chosen.
    ...
    matrix_of_ones[0:dim:acceleration,:] = 1 # Partialy samples the high k-space frequencies, by first sampling all frequencies skipping every few
    matrix_of_ones[int(dim/2-calib_size/2)-1:int(dim/2+calib_size/2)-1, :] = 1 # samples  continuous block of k-space low frequencies
